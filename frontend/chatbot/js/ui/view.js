...
export function createView(shadow, config) {
  ...
  root.innerHTML = `
    <button class="launcher" type="button" aria-expanded="false" aria-controls="chat">${chatIcon}<span>Ask <span data-name></span></span><span class="spark">âœ¦</span></button>
    <section id="chat" class="panel" role="dialog" aria-label="College chat assistant" hidden>
      <header><div class="avatar">${chatIcon}</div><div class="identity"><strong data-name></strong><span class="status" role="status">Connectingâ€¦</span></div>
        <button class="icon reset" type="button" aria-label="Reset conversation" title="Start a new conversation">â†»</button>
        <button class="icon close" type="button" aria-label="Close chat" title="Close chat">Ã—</button>
      </header>
      ...
      <form class="composer"><label class="sr-only" for="question">Your question</label><input id="question" maxlength="4000" autocomplete="off" placeholder="Ask about your next stepâ€¦"><button type="submit" class="send" aria-label="Send message">â†‘</button></form>
  ...
  return {
    ...
    message(message, onVote) {
      ...
      for (const link of message.action_links || []) {
        try {
          ...
          anchor.className = 'source'; anchor.textContent = `${link.title || 'View source'} â†—`;
    ...
    welcome(onSelect) {
      ...
      const description = document.createElement('p'); description.textContent = `Hi, Iâ€™m ${config.name}. Letâ€™s explore courses, enrolment and life at ${config.college}.`;
      ...
      for (const text of ['Explore courses', 'How do I enrol?', 'What is ATAR?', 'Student Services']) {
        const button = document.createElement('button'); button.type = 'button'; button.textContent = `${text} â†—`;
    ...
    busy(value) {
      ...
      find('.typing')?.remove();
      if (value) { const el = document.createElement('p'); el.className = 'typing'; el.textContent = `${config.name} is thinkingâ€¦`; el.setAttribute('role', 'status'); find('.messages').append(el); scroll(); }
    },
  };
}
